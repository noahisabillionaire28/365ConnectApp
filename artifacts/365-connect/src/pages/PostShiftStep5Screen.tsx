...
function SectionRow({
  ...
  return (
    <div className="flex items-start gap-3 py-2.5 border-b border-[#111] last:border-0">
      <div className="w-7 h-7 rounded-[8px] bg-[#141414] flex items-center justify-center flex-shrink-0 mt-0.5">
        {icon}
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-[#555] text-[11px] font-semibold uppercase tracking-wider mb-0.5">{label}</p>
        <p className="text-white text-[14px] font-medium leading-snug">{value}</p>
...
function CostRow({ label, value, highlight, sub }: { label: string; value: string; highlight?: boolean; sub?: boolean }) {
  return (
    <div className={`flex items-center justify-between ${sub ? 'opacity-60' : ''}`}>
      <p className={`text-[${sub ? '12' : '14'}px] ${highlight ? 'text-white font-bold text-[16px]' : 'text-[#666]'}`}>
        {label}
      </p>
      <p className={`font-${highlight ? 'bold' : 'medium'} text-[${highlight ? '20' : '14'}px] ${highlight ? 'text-primary' : 'text-[#888]'}`}>
        {value}
      </p>
    </div>
  );
...
export function PostShiftStep5Screen() {
  ...
  function fmtDate(iso: string) {
    if (!iso) return '';
    const d = new Date(`${iso}T12:00:00`); // noon to avoid timezone DST edge
  ...
  async function handlePost() {
    ...
    if (!startIso || !endIso) {
      setPostError('Invalid date or time — please go back and fix Step 2.');
      return;
    }

    // Find primary job type template for cover image
    ...
    try {
      await postShiftMutation.mutateAsync({
        ...
        company_name: undefined, // client fills their own name via profile
      ...
      // Show a brief native alert since toaster isn't wired to sonner yet
      // This will be upgraded to a proper sonner toast in P4
      setTimeout(() => {
        if (window.confirm !== undefined) {
          // noop — actual toast comes in P4
        }
      }, 0);

    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Failed to post shift. Try again.';
      setPostError(msg);
    }
  }
    ...
    <div className="min-h-[100dvh] bg-black flex flex-col">
      ...
      <div className="px-5 pt-5 pb-4 border-b border-[#111]">
        <div className="flex items-center gap-3 mb-4">
          <button
            type="button"
            aria-label="Back to shift details"
          ...
          <span className="text-[#555] text-[12px] font-medium flex-shrink-0">5 of 5</span>
        </div>
        <h1 className="text-white font-bold text-[22px] tracking-tight">Review & Post</h1>
        <p className="text-[#555] text-[13px] mt-1">Confirm your shift details before going live</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 pt-5 pb-36">
        ...
        {/* Shift summary card */}
        <section className="bg-[#0A0A0A] border border-[#1E1E1E] rounded-[18px] px-4 py-2 mb-5" aria-label="Shift summary">

          {/* Who */}
          <SectionRow
            icon={<Briefcase size={13} aria-hidden className="text-primary" />}
            label="Roles"
          ...
          {/* When */}
          <SectionRow
            icon={<Calendar size={13} aria-hidden className="text-primary" />}
            label="Date"
            value={draft.date ? fmtDate(draft.date) : '—'}
          />
          <SectionRow
            icon={<Clock size={13} aria-hidden className="text-primary" />}
            label="Time"
            value={draft.startTime ? `${fmtTime(draft.startTime)} – ${fmtTime(draft.endTime)}` : '—'}
            sub={durLabel !== '—' ? `${durLabel} shift` : undefined}
          />
          <SectionRow
            icon={<Repeat2 size={13} aria-hidden className="text-primary" />}
            label="Repeat"
          ...
          {/* Where */}
          <SectionRow
            icon={<MapPin size={13} aria-hidden className="text-primary" />}
            label="Location"
            value={draft.location || '—'}
            sub={draft.unit || undefined}
          />

          {/* Pay */}
          <SectionRow
            icon={<DollarSign size={13} aria-hidden className="text-primary" />}
            label="Pay Rate"
          ...
          {/* Headcount */}
          <SectionRow
            icon={<Users size={13} aria-hidden className="text-primary" />}
            label="Total Workers"
            value={String(totalW)}
          />
          ...
          {draft.dressCode && (
            <SectionRow
              icon={<Shirt size={13} aria-hidden className="text-primary" />}
              label="Dress Code"
          ...
          {/* Contact */}
          <SectionRow
            icon={<User size={13} aria-hidden className="text-primary" />}
            label="Contact"
        ...
        {/* Cost breakdown */}
        <section className="bg-[#0A0A0A] border border-[#1E1E1E] rounded-[18px] px-5 py-4 mb-5" aria-label="Estimated cost">
          ...
          <div className="flex flex-col gap-3">
            <CostRow
              label={`$${draft.payRate}/hr × ${totalW} worker${totalW !== 1 ? 's' : ''} × ${durLabel}`}
              value={`$${grossCost.toFixed(2)}`}
            />
            <CostRow
              label="365 Platform Fee (8%)"
        ...
        {/* Readiness checklist */}
        <section className="bg-[#0A0A0A] border border-[#1E1E1E] rounded-[18px] px-5 py-4 mb-6" aria-label="Readiness checklist">
          {[
            ...
            <div key={item.label} className="flex items-center gap-3 py-1.5">
              <CheckCircle2
                size={15}
                aria-hidden
                className={item.done ? 'text-emerald-400' : 'text-[#2A2A2A]'}
              />
              <span className={`text-[13px] ${item.done ? 'text-[#888]' : 'text-[#3A3A3A]'}`}>
                {item.label}
              </span>
            </div>
          ))}
        ...
        {postError && (
          <div
            className="mb-4 bg-red-500/10 border border-red-500/30 rounded-[12px] px-4 py-3"
            role="alert"
          >
            <p className="text-red-400 text-[13px] font-medium">{postError}</p>
          </div>
        )}
      </div>

      {/* Fixed CTA */}
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[390px] px-5 pb-9 pt-4 bg-gradient-to-t from-black via-black/98 to-transparent z-10">
        <motion.button
          type="button"
          whileTap={{ scale: 0.97 }}
          onClick={handlePost}
          ...
          aria-busy={postShiftMutation.isPending}
          className={`w-full h-[54px] rounded-[14px] font-bold text-[16px] transition-all flex items-center justify-center gap-2.5 ${
        ...
        >
          {postShiftMutation.isPending ? (
            <>
              <div className="w-4 h-4 rounded-full border-2 border-black/40 border-t-black animate-spin" aria-hidden />
              Posting…
            </>
          ) : (
            'Post Shift →'
