...
import {
  AlertTriangle, CheckCircle2, ShieldAlert, Ban,
  Clock, CheckCheck,
} from 'lucide-react';
...
import {
  ADMIN_DISPUTES,
  type AdminDispute, type DisputeStatus, type DisputeType,
} from '@/data/mockAdmin';

/* ─── Status & type metadata ────────────────────────────────────────────── */
const STATUS_META: Record<DisputeStatus, { label: string; cls: string; icon: React.ComponentType<{ size?: number; className?: string }> }> = {
  open:     { label: 'Open',     cls: 'bg-red-500/15    border-red-500/25    text-red-400',     icon: AlertTriangle  },
  resolved: { label: 'Resolved', cls: 'bg-emerald-500/15 border-emerald-500/25 text-emerald-400', icon: CheckCheck     },
  warned:   { label: 'Warned',   cls: 'bg-amber-500/15  border-amber-500/25  text-amber-400',   icon: ShieldAlert    },
  banned:   { label: 'Banned',   cls: 'bg-red-600/20    border-red-600/30    text-red-400',     icon: Ban            },
};
...
function ActionConfirm({
  action, disputeUserName, onConfirm, onCancel,
}: {
  ...
  const META = {
    ...
    warn: {
      ...
      btnClass: 'bg-amber-500 text-black',
    },
    ban: {
  ...
  return (
    ...
    >
      ...
      >
        <p className="text-white font-bold text-[17px] mb-1.5">{m.title}</p>
        <p className="text-[#555] text-[14px] mb-5 leading-relaxed">{m.body}</p>
        <div className="flex gap-3">
          ...
          <button
            type="button"
            onClick={onConfirm}
            className={`flex-1 h-[48px] rounded-[12px] font-bold text-[14px] ${m.btnClass}`}
          >
            {m.btnLabel}
          </button>
        </div>
...
function DisputeCard({
  ...
  onAction: (id: string, action: 'resolve' | 'warn' | 'ban') => void;
}) {
  ...
  return (
    <motion.div
      ...
      className={`border rounded-[16px] p-4 mb-3 ${
        isOpen
          ? 'bg-[#080808] border-red-500/12'
          : 'bg-[#040404] border-[#101010]'
      }`}
      role="listitem"
    >
      {/* Top row */}
      <div className="flex items-start justify-between mb-2">
        <div className="flex-1 min-w-0 pr-2">
          <div className="flex items-center gap-2 mb-0.5">
            <p className="text-white font-bold text-[15px]">{dispute.reportedUserName}</p>
      ...
      {/* Reason */}
      <div className={`rounded-[10px] px-3.5 py-2.5 mb-3 ${isOpen ? 'bg-[#100000] border border-red-500/10' : 'bg-[#080808] border border-[#0D0D0D]'}`}>
        <p className={`text-[13px] leading-relaxed ${isOpen ? 'text-[#AAAAAA]' : 'text-[#555]'}`}>
          {dispute.reason}
        </p>
      </div>
      ...
      <div className="flex items-center gap-1.5 mb-3">
        <Clock size={11} aria-hidden className="text-[#2A2A2A]" />
        <p className="text-[#2A2A2A] text-[11px]">{dispute.timestamp}</p>
      </div>
      ...
      {isOpen && (
        <div className="flex gap-2">
          <motion.button
            ...
            className="flex-1 h-[38px] rounded-[10px] bg-emerald-500/12 border border-emerald-500/25 text-emerald-400 text-[12px] font-bold flex items-center justify-center gap-1.5"
          >
          ...
          <motion.button
            ...
            className="flex-1 h-[38px] rounded-[10px] bg-amber-500/12 border border-amber-500/25 text-amber-400 text-[12px] font-bold flex items-center justify-center gap-1.5"
          >
          ...
          <motion.button
            ...
            className="flex-1 h-[38px] rounded-[10px] bg-red-500/12 border border-red-500/25 text-red-400 text-[12px] font-bold flex items-center justify-center gap-1.5"
          >
...
export function AdminDisputes() {
  ...
  function requestAction(id: string, action: 'resolve' | 'warn' | 'ban') {
  ...
  function confirmAction() {
    ...
    const newStatus: DisputeStatus =
      action === 'resolve' ? 'resolved' : action === 'warn' ? 'warned' : 'banned';
    setDisputes((prev) =>
  ...
  const pendingDispute = pendingAction
    ? disputes.find((d) => d.id === pendingAction.id)
    : null;
    ...
    <div className="min-h-[100dvh] bg-black flex flex-col">
      ...
      <div className="sticky top-[96px] z-30 bg-black border-b border-[#0D0D0D] px-4 pt-4 pb-3">
        {openCount > 0 && (
          <div
            role="status"
            aria-label={`${openCount} open disputes requiring attention`}
            className="flex items-center gap-2 bg-red-500/8 border border-red-500/20 rounded-[12px] px-3.5 py-2.5 mb-3"
          >
            <AlertTriangle size={14} aria-hidden className="text-red-400 flex-shrink-0" />
            <p className="text-red-400 text-[13px] font-semibold">
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar" role="group" aria-label="Filter disputes by status">
          {DISPUTE_FILTERS.map(({ key, label }) => (
            <button
              ...
              aria-label={`Filter disputes by ${label}`}
        ...
        <p className="text-[#2A2A2A] text-[11px] font-medium mt-2">{filtered.length} case{filtered.length !== 1 ? 's' : ''}</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-4 pb-12" role="list" aria-label="Dispute cases">
        ...
        {filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 gap-2">
            <CheckCircle2 size={32} aria-hidden className="text-[#1A1A1A]" />
            <p className="text-[#222] text-[14px]">No cases in this category.</p>
          </div>
        )}
      </div>
      ...
      <AnimatePresence>
        {pendingAction && pendingDispute && (
          <ActionConfirm
            key={`${pendingAction.id}-${pendingAction.action}`}
            action={pendingAction.action}
            disputeUserName={pendingDispute.reportedUserName}
