...
function FormLabel({ children }: { children: React.ReactNode }) {
  return (
    <p className="text-[#737373] text-[11px] font-semibold uppercase tracking-wider mb-1.5">{children}</p>
  );
}
...
function WorkerStepper({ label, count, onIncrement, onDecrement }: {
  label: string; count: number; onIncrement: () => void; onDecrement: () => void;
}) {
  return (
    <div className="flex items-center justify-between py-1">
      <span className="text-black text-[14px] font-medium">{label}</span>
      <div className="flex items-center gap-3">
        <button type="button" aria-label={`Decrease ${label} count`} onClick={onDecrement}
          className="w-8 h-8 rounded-[8px] bg-white border border-[#DBDBDB] flex items-center justify-center active:border-black">
          <Minus size={13} aria-hidden className="text-[#737373]" />
        </button>
        <span className="text-black font-bold text-[17px] w-6 text-center tabular-nums" aria-live="polite">
          {count}
        </span>
        <button type="button" aria-label={`Increase ${label} count`} onClick={onIncrement}
          className="w-8 h-8 rounded-[8px] bg-white border border-[#DBDBDB] flex items-center justify-center active:border-black">
          <Plus size={13} aria-hidden className="text-black" />
        </button>
      </div>
    </div>
  );
...
export function PostShiftStep2Screen() {
  ...
  function updateCount(type: string, delta: number) {
    setWorkerCounts((prev) => ({ ...prev, [type]: Math.max(1, Math.min(50, (prev[type] ?? 1) + delta)) }));
  }
    ...
    <div className="min-h-[100dvh] bg-white flex flex-col">
      ...
      <div className="px-5 pt-5 pb-4 border-b border-[#DBDBDB]">
        <div className="flex items-center gap-3 mb-4">
          <button type="button" aria-label="Back to job types" onClick={() => navigate('/post-shift/step1')}
          ...
          <span className="text-[#737373] text-[12px] font-medium flex-shrink-0">2 of 5</span>
        </div>
        <h1 className="text-black font-bold text-[22px] tracking-tight">When is it?</h1>
        <p className="text-[#737373] text-[13px] mt-1">Set the date, time, and staffing per role</p>
      </div>

      <div className="flex-1 overflow-y-auto px-5 pt-5 pb-36">
        <FormSection icon={<Calendar size={13} aria-hidden className="text-black" />} title="Date & Time">
          <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)}
                aria-label="Shift start time" className={INPUT_CLS} style={{ colorScheme: 'light' }} />
            </div>
            <div>
              <FormLabel>End time</FormLabel>
              <input type="time" value={endTime} onChange={(e) => setEndTime(e.target.value)}
                aria-label="Shift end time" className={INPUT_CLS} style={{ colorScheme: 'light' }} />
            </div>
          </div>
        ...
        <FormSection icon={<Repeat2 size={13} aria-hidden className="text-black" />} title="Repeat">
          <div className="flex items-center rounded-[8px] overflow-hidden border border-[#DBDBDB]"
            role="radiogroup" aria-label="Repeat frequency">
            {REPEAT_OPTIONS.map((opt, idx) => (
              <button key={opt.key} type="button" role="radio" aria-checked={repeat === opt.key}
                onClick={() => setRepeat(opt.key)}
                className={`flex-1 h-[40px] text-[13px] font-semibold transition-all duration-150
                  ${idx > 0 ? 'border-l border-[#DBDBDB]' : ''}
                  ${repeat === opt.key ? 'bg-black text-white' : 'bg-white text-[#737373] hover:text-black'}`}>
                {opt.label}
              </button>
            ))}
          </div>
        ...
        <FormSection icon={<Plus size={13} aria-hidden className="text-black" />} title="Workers Per Role">
          {initial.jobTypes.length === 0 ? (
            <p className="text-[#737373] text-[13px]">No job types selected — go back to Step 1.</p>
          ) : (
            <div className="flex flex-col divide-y divide-[#DBDBDB]" role="group" aria-label="Worker counts per role">
              {initial.jobTypes.map((jt) => (
                <WorkerStepper key={jt} label={jt} count={workerCounts[jt] ?? 1}
                  onIncrement={() => updateCount(jt, 1)} onDecrement={() => updateCount(jt, -1)} />
              ))}
            </div>
          )}
          {initial.jobTypes.length > 0 && (
            <div className="flex items-center justify-between pt-1 border-t border-[#DBDBDB]">
              <span className="text-[#737373] text-[12px] font-semibold uppercase tracking-wider">Total workers</span>
