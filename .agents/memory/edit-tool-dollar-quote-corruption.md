...
**How to apply:** After any Edit-tool write that touches `---
name: Edit tool corrupts $ in SQL dollar-quoting
description: Using the Edit tool on PL/pgSQL with $$ dollar-quoting can silently collapse "$$" to "$", producing invalid SQL that only fails at apply time.
---
...
-quoted SQL blocks, grep for lone `---
name: Edit tool corrupts $ in SQL dollar-quoting
description: Using the Edit tool on PL/pgSQL with $$ dollar-quoting can silently collapse "$$" to "$", producing invalid SQL that only fails at apply time.
---
...
**Why:** Caused every `CREATE FUNCTION ... AS $$ ... $$;` block introduced in one session to lose one `$`, breaking ~10 functions. Silent until SQL apply.
 ...
 where pairs were intended. Prefer `WriteFile` full-file rewrites over Edit-tool patches when adding many `---
name: Edit tool corrupts $ in SQL dollar-quoting
description: Using the Edit tool on PL/pgSQL with $$ dollar-quoting can silently collapse "$$" to "$", producing invalid SQL that only fails at apply time.
---
...
-quoted functions in one pass.

For TypeScript/JS seed scripts that need to *print* `$` SQL, do not write `'$'` as a string literal (the Edit tool will corrupt it). Instead use `String.fromCharCode(36, 36)` to build the two-character string at runtime, which survives edit tool corruption and bash PID substitution (`$` → process PID when passed through shell double-quotes).
